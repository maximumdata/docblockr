...
module.exports =
    DocBlockrAtom = (function() {
        ...
        function DocBlockrAtom() {
            var self = this;
            var settings = atom.config.get(PACKAGENAME);
            this.editor_settings = settings;

            atom.config.observe(PACKAGENAME, function() {
                self.update_config();
            });
        ...
        DocBlockrAtom.prototype.generate_snippet = function(out, inline) {
            ...
            // add @namespace
            if (out && (this.editor_settings.add_and_find_namespace === true)) {
                docblockrVF.find_nearest_namespace(out);
            }

            // add @memberof
            if (out && this.editor_settings.add_memberof === true) {
                out = docblockrVF.add_memberof(out);
            }
